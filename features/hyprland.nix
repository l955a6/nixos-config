{ pkgs, ... }: {
  wayland.windowManager.hyprland = {
    ...
    settings = {
      input = {
        kb_options = "ctrl:nocaps,ctrl:swap_rwin_rctl";
      };
      "$mainMod" = "SUPER";
      "$shift" = "SHIFT";
	...
	"$mainMod $shift, l, Move window right, movewindow, r"
	  ...
	  builtins.genList (i:
	    let workSpace = i + 1;
	    in [
	      "$mainMod, code:1${toString i}, Go to workspace ${toString workSpace}, workspace, ${toString workSpace}"
	      "$mainMod $shift, code:1${toString i}, Move window to ${toString workSpace}, movetoworkspacesilent, ${toString workSpace}"
	    ]
	  )
	  9
	)
